...
export default function CauseList(props) {
  ...
  useEffect(() => {
    axios(baseURL + 'causes/' + params.causeId)
      .then(async ({data}) => {
        setCauseData(data.data.cause);

        const promises = data.data.cause.volunteers.map(({volunteerId}) =>
  ...
  const handleDeleteVolunteer = (id) => {
    ...
    axios
      .patch(
        baseUrl + `causes/volunteers/${causeData._id}/delete/${id}`,
        {},
        config,
      )
        ...
        <Card plain>
          <CardHeader plain color="danger">
            <h4 className={classes.cardTitleWhite}>
              Verify volunteers of "{causeData.name}"
            </h4>
