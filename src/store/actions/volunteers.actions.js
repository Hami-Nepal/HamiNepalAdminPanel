...
export const listVolunteers = (page) => (dispatch) => {
  dispatch({type: VOLUNTEERS_LIST});
  try {
    VolunteerService.volunteerList(page).then(
      (resp) => {
...
export const volunteerSignUp = (volunteer) => (dispatch) => {
  ...
  try {
    VolunteerService.volunteerSignUp(volunteer).then(
      (resp) => {
        dispatch({
...
export const verifyVolunteer = (id) => (dispatch) => {
  ...
  try {
    VolunteerService.verifyVolunteer(id).then(
      (resp) => {
        dispatch({
